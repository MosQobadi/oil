...
export function CarFinderSection() {
  ...
  const getRecommendedProducts = () => {
    ...
    const oilProduct =
      allProducts.find(
        (p) => p.id === selectedEngine.recommendedOil.productId,
      ) ||
      allProducts.find(
        (p) =>
          p.category === "engine-oil" &&
          p.viscosity === selectedEngine.recommendedOil.viscosity,
      );

    const oilFilter = allProducts.find(
      (p) => p.category === "oil-filter" && p.badge === "Premium",
    );
    const airFilter = allProducts.find(
      (p) => p.category === "air-filter" && p.badge === "Standard",
    );
    const cabinFilter = allProducts.find(
      (p) => p.category === "cabin-filter" && p.badge === "Premium",
    );
              ...
              >
                ...
                <div className={isRTL ? "text-right" : ""}>
                  ...
                  <h3 className="text-xl font-bold text-foreground">
                    {selectedYear}{" "}
                    {language === "fa" && selectedBrand
                      ? selectedBrand.nameFA
                      : selectedBrand?.name}{" "}
                    {selectedModel?.name}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {selectedEngine.name} ({selectedEngine.displacement})
                  </p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {recommendedProducts.oilProduct && (
                  <ProductRecommendationCard
                    ...
                    specs={{
                      [t.carFinder.viscosity]:
                        selectedEngine.recommendedOil.viscosity,
                      [t.carFinder.capacity]:
                        selectedEngine.recommendedOil.capacity,
                      [t.carFinder.specification]:
                        selectedEngine.recommendedOil.specification,
                    }}
                    onAddToCart={() =>
                      handleAddToCart(recommendedProducts.oilProduct!)
                    }
                    language={language}
                    t={t}
                    isRTL={isRTL}
                ...
                {recommendedProducts.oilFilter && (
                  <ProductRecommendationCard
                    ...
                    specs={{
                      "Part #":
                        selectedEngine.recommendedFilters.oilFilter.partNumber,
                    }}
                    onAddToCart={() =>
                      handleAddToCart(recommendedProducts.oilFilter!)
                    }
                    language={language}
                    t={t}
                    isRTL={isRTL}
                ...
                {recommendedProducts.airFilter && (
                  <ProductRecommendationCard
                    ...
                    specs={{
                      "Part #":
                        selectedEngine.recommendedFilters.airFilter.partNumber,
                    }}
                    onAddToCart={() =>
                      handleAddToCart(recommendedProducts.airFilter!)
                    }
                    language={language}
                    t={t}
                    isRTL={isRTL}
                ...
                {recommendedProducts.cabinFilter && (
                  <ProductRecommendationCard
                    ...
                    specs={{
                      "Part #":
                        selectedEngine.recommendedFilters.cabinFilter
                          .partNumber,
                    }}
                    onAddToCart={() =>
                      handleAddToCart(recommendedProducts.cabinFilter!)
                    }
                    language={language}
                    t={t}
                    isRTL={isRTL}
...
function ProductRecommendationCard({
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <h4
          className={`font-bold text-foreground mb-1 ${isRTL ? "text-right" : ""}`}
        >
          {language === "fa" ? product.nameFA : product.name}
        </h4>
        <p
          className={`text-xs text-muted-foreground mb-3 line-clamp-2 ${isRTL ? "text-right" : ""}`}
        >
        ...
        {/* Price & CTA */}
        <div
          className={`flex items-center justify-between pt-3 border-t border-border/30 ${isRTL ? "flex-row-reverse" : ""}`}
        >
          <div className={isRTL ? "text-right" : ""}>
            ...
            {product.originalPrice && (
              <span
                className={`text-xs text-muted-foreground line-through ${isRTL ? "mr-2" : "ml-2"}`}
              >
                ${product.originalPrice}
              </span>
            )}
